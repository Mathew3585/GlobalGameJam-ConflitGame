...
namespace AdvancedShooterKit.Utils
{
    ...
    public static class ComponentExtensions
    {
        ...
        {
            return new
                GameObject( string.Format( name ), EmptyMono.type )
                .MoveToCache()
                .GetComponent<MonoBehaviour>();
        }

        // Spawn Copy
        public static T SpawnCopy<T>( this T original, Transform point ) where T : UObject
        {
            return UObject.Instantiate( original, point.position, point.rotation );
        }
